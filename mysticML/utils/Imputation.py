...
class Imputation(Daddy):
    ...
    def apply(self) -> pandas.DataFrame:
        ...
        assert (
            self.target not in col_drop
        ), f"{self.target} is filled mostly with null values ; cannot be a target column."
        self.x.drop(col_drop, axis=1, inplace=True)
        self.x.dropna(subset=[self.target], axis=0, inplace=True)
        rows_with_majority_nan = self.x[
            (self.x.isna().sum(axis=1) > (self.x.shape[1] / 2)).values
        ]
        ...
        for col in self.x.columns:
            if (self.x[col].dtype == "object") or (
                self.x[col].dtype == "int64"
                and self.x[col].nunique() <= 11
                and self.x[col].min() >= 0
            ):
                flag = 1
